...
Hard dependencies: `docs/features/01_model_management/`, `docs/integrations/02_vllm.md`, `docs/integrations/03_ollama.md`, `docs/integrations/04_llamacpp.md`. If any hard dependency is unavailable, Provider Abstraction fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
**Success:** `admin` calls `POST /api/v1/inference-gateway/provider-abstraction` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `inference_gateway.provider_abstraction` audit event recorded.
