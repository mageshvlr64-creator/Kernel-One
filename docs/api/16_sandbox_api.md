...
|---|---|---|---|---|
| `POST` | `/api/v1/sandbox/execute` | {task_id, language, code} | ToolInvocationResult (tool_id='code_execution.run') | `network_required=false` enforced at the container level, not just this endpoint (REQ-SEC-002). |
| `GET` | `/api/v1/sandbox/executions/{id}/logs` | (none) | {stdout, stderr, exit_code} | — |

## Example — POST /api/v1/sandbox/execute
