...
**Permission Filtering** is the unit of Knowledge Fabric responsible for indexing, retrieving, and reranking document content for RAG as it specifically relates to "permission filtering." It exists as its own document because it has its own inputs, its own failure modes, and its own permission boundary — distinct from the other files in `docs/features/13_knowledge_fabric/`.

## 2. Scope

In scope: validating and executing the `permission filtering` operation, updating the `chunks / embeddings` store, and emitting the corresponding audit event (`knowledge_fabric.permission_filtering`). Out of scope: anything owned by a sibling file in this feature group, and anything listed under Non-goals below.

## 3. Non-goals

Permission Filtering does not perform its own permission check logic — it calls the shared RBAC layer (`docs/features/19_identity_and_rbac/05_permissions.md`). It does not decide routing or model selection itself. It does not write directly to the audit table — it emits an event that `docs/features/17_audit/` consumes. It makes zero outbound network calls outside the active network mode.

## 4. User-facing behavior

From the workbench UI, permission filtering is triggered by the corresponding action in the relevant panel (see `docs/ui/`) or issued programmatically via `POST /api/v1/knowledge-fabric/permission-filtering`. On success the UI reflects the new state within one refresh cycle (≤ 1s). On failure the UI shows a specific, human-readable message mapped from the error code table below — never a raw stack trace or a silent no-op.

## 5. System behavior

1) Request arrives at `/api/v1/knowledge-fabric/permission-filtering` and passes through auth + RBAC + policy checks. 2) The `Knowledge Fabric` service validates the payload against its schema. 3) The operation is applied to the `chunks / embeddings` store inside a single transaction. 4) An audit event `knowledge_fabric.permission_filtering` is emitted. 5) The response is returned to the caller. Steps 3-4 are atomic: if step 4 cannot be guaranteed, step 3 is rolled back rather than leaving an unaudited state change.
...
On success: the `chunks / embeddings` row reflects the new state; exactly one audit event `knowledge_fabric.permission_filtering` exists for this invocation; any cache or index derived from `chunks / embeddings` is invalidated or updated in the same logical operation. On failure: no partial write is visible to any other caller.

## 10. Data structures

Canonical shape lives in `docs/schemas/` (see the schema file matching `chunks / embeddings`) and `docs/domain/` for the entity-level definition. This file does not redefine those fields — it only specifies which of them `Permission Filtering` reads or writes.

## 11. API contracts

`POST /api/v1/knowledge-fabric/permission-filtering` — create/invoke. `GET /api/v1/knowledge-fabric/permission-filtering/{id}` — read current state. Full request/response schema, headers, and pagination (where applicable) are defined in `docs/api/` under the entry for `Knowledge Fabric`. Auth: Bearer session token, required on every call, no anonymous access.

## 12. Internal interfaces

Backend module: `knowledge_fabric_permission_filtering_service`, exposing a single entry point `handle_permission_filtering(context, payload) -> Result`. Callers (e.g. the agent kernel's tool selection step, or the admin console) invoke this function directly for in-process calls; the HTTP route above is a thin wrapper around the same function — there is exactly one implementation, not two.

## 13. State transitions

Permission Filtering moves its resource through the states defined in the relevant `docs/runtime/` state machine file for `Knowledge Fabric`. It is only permitted to trigger the specific transition(s) relevant to "permission filtering" — any other transition is a defect, and the state machine layer must reject it rather than silently allow it.
...
On failure, no partial state from Permission Filtering is left visible. If Permission Filtering is a step inside an agent plan, the agent kernel's replanning logic (`docs/features/04_agent_kernel/10_replanning.md`) decides whether to retry, substitute a different approach, or surface the failure to the user, per `docs/runtime/14_resume_and_recovery.md`.

## 21. Observability requirements

Emits one structured log line per invocation (`level`, `event=knowledge_fabric.permission_filtering`, `actor_id`, `duration_ms`, `outcome`) and one metric `knowledge_fabric_permission_filtering_duration_seconds` (histogram) plus `knowledge_fabric_permission_filtering_total{outcome}` (counter), per `docs/features/25_observability/03_metrics.md`.
...
Minimum coverage: one unit test per row in the Failure modes table above, one integration test for the full success path through `/api/v1/knowledge-fabric/permission-filtering`, and one permission test per role in the table above (three roles × allow/deny = at least 3 assertions), per `docs/testing/01_testing_strategy.md`.

## 25. Acceptance criteria

- [ ] `POST /api/v1/knowledge-fabric/permission-filtering` succeeds for an `admin` caller with a valid payload and returns the shape in Outputs.
- [ ] The same call is denied per the Permission requirements table for a disallowed role, with the correct error code.
- [ ] Exactly one `knowledge_fabric.permission_filtering` audit event is produced per invocation, success or failure.
