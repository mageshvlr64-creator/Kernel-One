...
**Success:** `admin` calls `POST /api/v1/audit/artifact-logging` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `audit.artifact_logging` audit event recorded.
