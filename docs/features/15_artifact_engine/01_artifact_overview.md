...
> Feature group: **Artifact Engine** (`docs/features/15_artifact_engine/`) · File: `01_artifact_overview.md`
> Part of the Sovereign AI Workbench (SIH26176) specification set. Risk level: **medium**.
> Previous: _(first document in this feature)_ · Next: `02_artifact_lifecycle.md`

## 1. Purpose

**Artifact Overview** is the unit of Artifact Engine responsible for generating downloadable Office/PDF/data files from agent output as it specifically relates to "artifact overview." It exists as its own document because it has its own inputs, its own failure modes, and its own permission boundary — distinct from the other files in `docs/features/15_artifact_engine/`.

## 2. Scope

In scope: validating and executing the `artifact overview` operation, updating the `artifacts` store, and emitting the corresponding audit event (`artifact_engine.artifact_overview`). Out of scope: anything owned by a sibling file in this feature group, and anything listed under Non-goals below.

## 3. Non-goals

Artifact Overview does not perform its own permission check logic — it calls the shared RBAC layer (`docs/features/19_identity_and_rbac/05_permissions.md`). It does not decide routing or model selection itself. It does not write directly to the audit table — it emits an event that `docs/features/17_audit/` consumes. It makes zero outbound network calls outside the active network mode.

## 4. User-facing behavior

From the workbench UI, artifact overview is triggered by the corresponding action in the relevant panel (see `docs/ui/`) or issued programmatically via `POST /api/v1/artifact-engine/artifact-overview`. On success the UI reflects the new state within one refresh cycle (≤ 1s). On failure the UI shows a specific, human-readable message mapped from the error code table below — never a raw stack trace or a silent no-op.

## 5. System behavior

1) Request arrives at `/api/v1/artifact-engine/artifact-overview` and passes through auth + RBAC + policy checks. 2) The `Artifact Engine` service validates the payload against its schema. 3) The operation is applied to the `artifacts` store inside a single transaction. 4) An audit event `artifact_engine.artifact_overview` is emitted. 5) The response is returned to the caller. Steps 3-4 are atomic: if step 4 cannot be guaranteed, step 3 is rolled back rather than leaving an unaudited state change.
...
On success: the `artifacts` row reflects the new state; exactly one audit event `artifact_engine.artifact_overview` exists for this invocation; any cache or index derived from `artifacts` is invalidated or updated in the same logical operation. On failure: no partial write is visible to any other caller.

## 10. Data structures

Canonical shape lives in `docs/schemas/` (see the schema file matching `artifacts`) and `docs/domain/` for the entity-level definition. This file does not redefine those fields — it only specifies which of them `Artifact Overview` reads or writes.

## 11. API contracts

`POST /api/v1/artifact-engine/artifact-overview` — create/invoke. `GET /api/v1/artifact-engine/artifact-overview/{id}` — read current state. Full request/response schema, headers, and pagination (where applicable) are defined in `docs/api/` under the entry for `Artifact Engine`. Auth: Bearer session token, required on every call, no anonymous access.

## 12. Internal interfaces

Backend module: `artifact_engine_artifact_overview_service`, exposing a single entry point `handle_artifact_overview(context, payload) -> Result`. Callers (e.g. the agent kernel's tool selection step, or the admin console) invoke this function directly for in-process calls; the HTTP route above is a thin wrapper around the same function — there is exactly one implementation, not two.

## 13. State transitions

Artifact Overview moves its resource through the states defined in the relevant `docs/runtime/` state machine file for `Artifact Engine`. It is only permitted to trigger the specific transition(s) relevant to "artifact overview" — any other transition is a defect, and the state machine layer must reject it rather than silently allow it.
...
On failure, no partial state from Artifact Overview is left visible. If Artifact Overview is a step inside an agent plan, the agent kernel's replanning logic (`docs/features/04_agent_kernel/10_replanning.md`) decides whether to retry, substitute a different approach, or surface the failure to the user, per `docs/runtime/14_resume_and_recovery.md`.

## 21. Observability requirements

Emits one structured log line per invocation (`level`, `event=artifact_engine.artifact_overview`, `actor_id`, `duration_ms`, `outcome`) and one metric `artifact_engine_artifact_overview_duration_seconds` (histogram) plus `artifact_engine_artifact_overview_total{outcome}` (counter), per `docs/features/25_observability/03_metrics.md`.
...
Minimum coverage: one unit test per row in the Failure modes table above, one integration test for the full success path through `/api/v1/artifact-engine/artifact-overview`, and one permission test per role in the table above (three roles × allow/deny = at least 3 assertions), per `docs/testing/01_testing_strategy.md`.

## 25. Acceptance criteria

- [ ] `POST /api/v1/artifact-engine/artifact-overview` succeeds for an `admin` caller with a valid payload and returns the shape in Outputs.
- [ ] The same call is denied per the Permission requirements table for a disallowed role, with the correct error code.
- [ ] Exactly one `artifact_engine.artifact_overview` audit event is produced per invocation, success or failure.
...
Artifact Overview is done when all Acceptance criteria above pass in CI, the corresponding `docs/schemas/` and `docs/api/` entries match the implementation exactly, and the checklist in `docs/11_DEFINITION_OF_DONE.md` is satisfied.

## 27. Implementation notes

Implement `artifact_engine.artifact_overview` as a single service function called by both the HTTP route and any internal caller (e.g. the agent kernel) — do not duplicate the logic. Reuse the shared RBAC/policy check helper rather than writing a new role comparison; reuse the shared audit-emit helper rather than writing to the audit table directly.
