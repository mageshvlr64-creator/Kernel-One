...
> Root specification document · `docs/07_HARDWARE_AND_DEPLOYMENT_CONSTRAINTS.md`
> Previous: `06_TECHNOLOGY_STACK.md` · Next: `08_BUILD_PHASES.md`

## Purpose

This document defines the hardware envelope this system must run within, and how that constrains model and feature choices for the Sovereign AI Workbench (SIH26176).
...
1. **Statement.** Hardware and Deployment Constraints is authoritative for its topic across the entire Sovereign AI Workbench
...
2. **Cross-cutting application.** Every feature group under `docs/features/` is expected to be
   consistent with Hardware and Deployment Constraints — if a reviewer finds a feature file that conflicts with this
...
3. **Enforcement.** Adherence to Hardware and Deployment Constraints is part of `11_DEFINITION_OF_DONE.md` and
...
Changes to Hardware and Deployment Constraints must be reflected in `docs/20_DECISION_LOG.md` with the date, the reason
