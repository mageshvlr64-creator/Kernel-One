...
Hard dependencies: `docs/features/02_model_router/`, `docs/features/05_tool_gateway/`, `docs/features/16_human_approval/`, `docs/features/22_agent_memory/`. If any hard dependency is unavailable, Agent Kernel Overview fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
Resource: `Task` · Action: `read` · Risk level: **medium**.
...
Every invocation, successful or not, produces exactly one `AuditEvent` of type `agent_kernel.agent_kernel_overview` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `Task`; `action` is `read`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).
...
**Success:** `admin` calls `POST /api/v1/agent-kernel/agent-kernel-overview` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `agent_kernel.agent_kernel_overview` audit event recorded.
