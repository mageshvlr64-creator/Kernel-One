...
Hard dependencies: `docs/integrations/10_libreoffice.md`, `docs/features/14_evidence_and_provenance/`. If any hard dependency is unavailable, Artifact Failures fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
Every invocation, successful or not, produces exactly one `AuditEvent` of type `artifact_engine.artifact_failures` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `Artifact`; `action` is `execute`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).
...
**Success:** `admin` calls `POST /api/v1/artifact-engine/artifact-failures` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `artifact_engine.artifact_failures` audit event recorded.
