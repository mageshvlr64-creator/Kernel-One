...
- **Denied-POST connection aborts fixed in all three stdlib-HTTP services (Character 3).** evidence-service, knowledge-fabric and document-pipeline authenticated POSTs before reading the request body, so denials were written while the client was still sending body bytes; closing with unread input aborts connections on Windows (WinError 10053) under load — seen as a 1-in-5 full-suite flake in evidence-service. `_deny` now drains only the unread remainder (tracked via the body reader) before responding. Raw-socket mid-body regression test added per service — verified to fail against the pre-fix code. Counts: evidence 123, doc-pipeline 94, knowledge-fabric 57, repo 552.

- **Every service declares its test dependencies (Character 3).** Each service now ships a requirements-test.txt (pytest; pytest-asyncio where suites have async tests), and the three services that had none got an explicit requirements.txt — document-pipeline's real runtime dep (pymupdf) moved there, while knowledge-fabric and evidence-service document that they are standard-library only. CI's install step no longer improvises (ad-hoc pytest/pytest-asyncio/pymupdf lines removed): it installs exactly requirements.txt + requirements-test.txt, so a suite importing an undeclared package is now a loud install failure instead of a silent pass on a developer machine. Verified in a bare environment with only the declared files installed; YAML validated; repo runner green.

- **Model registry load is strict and atomic (Character 3).** ModelRegistry.load() previously logged and silently dropped malformed models.json entries (and duplicate ids overwrote last-wins), so a broken catalog half-loaded invisibly at boot. It now raises RegistryConfigError on invalid JSON, a non-object root, a missing/non-list "models" key, any malformed entry (error names the index and id), or a duplicate id (error names both indexes); the registry only mutates after every entry validates, and unset-path/missing-file keep their documented start-empty behavior. 12 strictness tests added (model-router 70 -> 82, repo 552 -> 564).

- **RESTRICTED-cleared model, hardened admin API, and clean boot failure (Character 3).** The catalog gains deepseek-r1-distill-32b (vllm, AWQ-INT4, 64k ctx, active_parameters set for the MoE), the first RESTRICTED-cleared entry, so the top classification is actually routable end-to-end. POST /api/v1/models now enforces Model's field constraints (ge bounds on parameters/context) and maps unknown capability tags to 422 INVALID_REQUEST instead of an unhandled 500; a malformed models.json now produces a critical log plus SystemExit with an actionable message instead of an opaque traceback. 20 tests: 12 admin-validation (4xx enforcement, registry unchanged after rejection, role checks) and 8 end-to-end routing-policy (RESTRICTED reaches only the cleared model, empty fallback chain proves no down-level leak, confidential excludes INTERNAL-only llava, catalog-drift mirror of models.json). model-router 82 -> 102, repo 564 -> 584.

- **model-router requirements-test.txt gains pytest-asyncio (Character 3).** The new strictness tests made ModelRegistry.load() calls that are async, so CI's model-router leg — which now installs exactly the declared files — failed at collection. The declaration was stale the moment an async test landed; this is the undeclared-import trap catching its own authors, fixed by declaring it.

- **Health-polling and circuit-breaker chain pinned end-to-end (Character 3).** New test_health_breaker_e2e.py drives the real app singletons through the FastAPI routes: the registry's actual _check_all_health code path (provider endpoints faked via httpx.MockTransport) flips is_available, unhealthy models become unselectable (503) and recover on a healthy poll, and breaker behavior is exercised over report-result plus selection: threshold trip with fallback to the next-ranked model, failure-count reset on success, 60s window expiry forgiving old failures, half-open probe success closing and failure re-opening the circuit, and per-model independence. 12 tests added (model-router 102 -> 114, repo 584 -> 596); as-built row updated.

- **Circuit-breaker window reset made clock-independent (Character 3).** The new e2e suite's window-expiry test passed locally but failed on CI's Linux runner with the breaker stuck OPEN. Root cause: record_failure used last_failure_time > 0.0 as the "has failed before" guard, but time.monotonic() reads seconds since boot, so on a freshly booted host a real timestamp can land below the 60s window — indistinguishable from the 0.0 "never failed" sentinel, so the window-reset branch never fired and the breaker counted failures ever instead of in-window exactly where uptime was shortest. last_failure_time is now Optional[float] with None meaning never-failed; no absolute clock reading participates in any decision. Found by CI, fixed, model-router 114/114 and repo runner 596/596 green.

- **Fresh-boot regression tests pin the clock-sentinel fix; repo-wide audit found no other instances (Character 3).** Two tests in test_circuit_breaker.py simulate a host ~30s past boot by patching monotonic(): a 31s-backdated failure lands negative, which the old > 0.0 guard misread as never-failed — the fixed code forgives the out-of-window failures and still trips on in-window ones, and a sentinel test pins None as the only never-failed representation. A sweep of all six services for sentinel time comparisons (0.0/-1/empty timestamps checked against monotonic or wall clocks) found no other instance: every other clock use is a pure delta (latency measurement) or an audit-trail record field. model-router 114 -> 116, repo total 596 -> 598.

- **Poll-loop timing made testable without real sleeps; opened_at joins the None-sentinel convention (Character 3).** ModelRegistry now accepts an injectable sleep (default asyncio.sleep), so the health-poll loop's timing is verified with a fake that records intervals and yields one tick: the configured interval is honored each round, every registered model is checked per round via a stub HTTP client (health results written, availability synced), and stop_health_polling cancels the loop cleanly — three tests, zero real delays. The first version hung the suite by hot-spinning the loop with a no-op fake; the yield requirement is documented in the test. circuit_breaker.py's opened_at now also uses Optional None (never-opened) instead of 0.0, with the OPEN branch guarding it conservatively — every timestamp in the module follows one sentinel convention. model-router 116 -> 119, repo total 598 -> 601.

- **Injectable clocks end real-delay testing across model-router and inference-gateway (Character 3).** CircuitBreaker takes an optional clock callable (default time.monotonic): its two real-sleep unit tests now advance a FakeClock instantly, so the breaker's time-dependent surface has zero sleeps. InferenceGateway takes sleep (retry backoff) and wait_for (timeout budgets): new tests record the configured backoff interval per retryable failure (and that non-retryable codes fail over with none), prove vision requests get the vision budget and text requests the text budget, map instant timeout expiry to INFERENCE_TIMEOUT, and — a previously untested surface — drive all three real adapters through httpx.MockTransport to verify HTTP timeouts map to INFERENCE_TIMEOUT and health checks report unhealthy. The existing vision-budget test migrated from monkeypatching app.gateway.asyncio to constructor injection. inference-gateway 25 -> 36, repo total 601 -> 612.
