...
This document defines the design principles (gateway pattern, fail-closed, audit-by-construction, sovereignty-by-default) behind the architecture for the Sovereign AI Workbench (SIH26176).
...
1. **Statement.** Architectural Principles is authoritative for its topic across the entire Sovereign AI Workbench
...
2. **Cross-cutting application.** Every feature group under `docs/features/` is expected to be
   consistent with Architectural Principles — if a reviewer finds a feature file that conflicts with this
...
3. **Enforcement.** Adherence to Architectural Principles is part of `11_DEFINITION_OF_DONE.md` and
...
Changes to Architectural Principles must be reflected in `docs/20_DECISION_LOG.md` with the date, the reason
