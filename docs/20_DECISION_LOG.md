...
> Root specification document · `docs/20_DECISION_LOG.md`
> Previous: `19_GLOSSARY.md` · Next: _(first directory: `architecture/`)_

## Purpose

This document defines the running record of non-obvious decisions made during design and implementation for the Sovereign AI Workbench (SIH26176).
...
1. **Statement.** Decision Log is authoritative for its topic across the entire Sovereign AI Workbench
...
2. **Cross-cutting application.** Every feature group under `docs/features/` is expected to be
   consistent with Decision Log — if a reviewer finds a feature file that conflicts with this
...
3. **Enforcement.** Adherence to Decision Log is part of `11_DEFINITION_OF_DONE.md` and
...
Changes to Decision Log must be reflected in `docs/20_DECISION_LOG.md` with the date, the reason
