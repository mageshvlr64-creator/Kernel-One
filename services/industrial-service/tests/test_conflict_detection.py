...
def _claim(document_id, document_name, authority, parameter, value,
...
class TestDetectConflicts:
    def test_no_conflict_when_values_agree(self):
        claims = [
            _claim(DOC_A_ID, "SOP-204", "primary", "filter replacement interval",
                   "30 days", date(2024, 1, 1)),
            _claim(DOC_B_ID, "SOP-205", "primary", "filter replacement interval",
                   "30 days", date(2024, 1, 1)),
        ]
    ...
    def test_genuine_conflict_detected_and_unresolved(self):
        ...
        """
        claims = [
            _claim(DOC_A_ID, "SOP-204 Rev.7", "primary", "filter replacement interval",
                   "30 days", date(2024, 3, 1)),
            _claim(DOC_B_ID, "MM-118", "primary", "filter replacement interval",
                   "45 days", date(2022, 1, 1)),
        ]
        conflicts = detect_conflicts(EQUIPMENT_ID, claims)
    ...
    def test_non_overlapping_validity_windows_no_conflict(self):
        ...
        """
        claims = [
            _claim(DOC_A_ID, "Old SOP", "primary", "filter replacement interval",
                   "30 days",
                   effective_from=date(2018, 1, 1),
                   effective_until=date(2021, 12, 31)),
            _claim(DOC_B_ID, "New SOP", "primary", "filter replacement interval",
                   "45 days",
    ...
    def test_different_parameters_no_conflict(self):
        claims = [
            _claim(DOC_A_ID, "SOP-A", "primary", "filter replacement interval",
                   "30 days", date(2024, 1, 1)),
            _claim(DOC_B_ID, "SOP-B", "primary", "operating temperature",
                   "120C", date(2024, 1, 1)),
        ]
        conflicts = detect_conflicts(EQUIPMENT_ID, claims)
        assert conflicts == []
    ...
    def test_synonym_normalization_triggers_conflict(self):
        ...
        claims = [
            _claim(DOC_A_ID, "SOP-A", "primary", "torque", "150 Nm",
                   date(2024, 1, 1)),
            _claim(DOC_B_ID, "MM-B", "primary", "bolt torque", "120 Nm",
                   date(2024, 1, 1)),
        ]
        conflicts = detect_conflicts(EQUIPMENT_ID, claims)
    ...
    def test_conflict_never_auto_resolved(self):
        ...
        claims = [
            _claim(DOC_A_ID, "SOP", "primary", "interval", "30 days",
                   date(2024, 1, 1)),
            _claim(DOC_B_ID, "Manual", "primary", "interval", "45 days",
                   date(2024, 1, 1)),
        ]
        conflicts = detect_conflicts(EQUIPMENT_ID, claims)
