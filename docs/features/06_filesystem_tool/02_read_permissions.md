...
**Success:** `admin` calls `POST /api/v1/filesystem-tool/read-permissions` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `filesystem_tool.read_permissions` audit event recorded.
