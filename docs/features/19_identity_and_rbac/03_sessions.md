...
**Success:** `admin` calls `POST /api/v1/identity-and-rbac/sessions` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `identity_and_rbac.sessions` audit event recorded.
