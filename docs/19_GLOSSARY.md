...
> Root specification document · `docs/19_GLOSSARY.md`
> Previous: `18_DOCUMENTATION_INDEX.md` · Next: `20_DECISION_LOG.md`

## Purpose

This document defines the precise meaning of every term used consistently across this documentation set for the Sovereign AI Workbench (SIH26176).
...
1. **Statement.** Glossary is authoritative for its topic across the entire Sovereign AI Workbench
...
2. **Cross-cutting application.** Every feature group under `docs/features/` is expected to be
   consistent with Glossary — if a reviewer finds a feature file that conflicts with this
...
3. **Enforcement.** Adherence to Glossary is part of `11_DEFINITION_OF_DONE.md` and
...
Changes to Glossary must be reflected in `docs/20_DECISION_LOG.md` with the date, the reason
