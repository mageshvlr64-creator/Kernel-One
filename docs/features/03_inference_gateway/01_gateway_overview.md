...
**Gateway Overview** is the unit of Inference Gateway responsible for the gateway normalizing calls to vLLM, Ollama, and llama.cpp behind one OpenAI-compatible interface as it specifically relates to "gateway overview." It exists as its own document because it has its own inputs, its own failure modes, and its own permission boundary — distinct from the other files in `docs/features/03_inference_gateway/`.

## 2. Scope

In scope: validating and executing the `gateway overview` operation, updating the `inference_requests` store, and emitting the corresponding audit event (`inference_gateway.gateway_overview`). Out of scope: anything owned by a sibling file in this feature group, and anything listed under Non-goals below.

## 3. Non-goals

Gateway Overview does not perform its own permission check logic — it calls the shared RBAC layer (`docs/features/19_identity_and_rbac/05_permissions.md`). It does not decide routing or model selection itself. It does not write directly to the audit table — it emits an event that `docs/features/17_audit/` consumes. It makes zero outbound network calls outside the active network mode.

## 4. User-facing behavior

From the workbench UI, gateway overview is triggered by the corresponding action in the relevant panel (see `docs/ui/`) or issued programmatically via `POST /api/v1/inference-gateway/gateway-overview`. On success the UI reflects the new state within one refresh cycle (≤ 1s). On failure the UI shows a specific, human-readable message mapped from the error code table below — never a raw stack trace or a silent no-op.

## 5. System behavior

1) Request arrives at `/api/v1/inference-gateway/gateway-overview` and passes through auth + RBAC + policy checks. 2) The `Inference Gateway` service validates the payload against its schema. 3) The operation is applied to the `inference_requests` store inside a single transaction. 4) An audit event `inference_gateway.gateway_overview` is emitted. 5) The response is returned to the caller. Steps 3-4 are atomic: if step 4 cannot be guaranteed, step 3 is rolled back rather than leaving an unaudited state change.
...
On success: the `inference_requests` row reflects the new state; exactly one audit event `inference_gateway.gateway_overview` exists for this invocation; any cache or index derived from `inference_requests` is invalidated or updated in the same logical operation. On failure: no partial write is visible to any other caller.

## 10. Data structures

Canonical shape lives in `docs/schemas/` (see the schema file matching `inference_requests`) and `docs/domain/` for the entity-level definition. This file does not redefine those fields — it only specifies which of them `Gateway Overview` reads or writes.

## 11. API contracts

`POST /api/v1/inference-gateway/gateway-overview` — create/invoke. `GET /api/v1/inference-gateway/gateway-overview/{id}` — read current state. Full request/response schema, headers, and pagination (where applicable) are defined in `docs/api/` under the entry for `Inference Gateway`. Auth: Bearer session token, required on every call, no anonymous access.

## 12. Internal interfaces

Backend module: `inference_gateway_gateway_overview_service`, exposing a single entry point `handle_gateway_overview(context, payload) -> Result`. Callers (e.g. the agent kernel's tool selection step, or the admin console) invoke this function directly for in-process calls; the HTTP route above is a thin wrapper around the same function — there is exactly one implementation, not two.

## 13. State transitions

Gateway Overview moves its resource through the states defined in the relevant `docs/runtime/` state machine file for `Inference Gateway`. It is only permitted to trigger the specific transition(s) relevant to "gateway overview" — any other transition is a defect, and the state machine layer must reject it rather than silently allow it.

## 14. Dependencies

Hard dependencies: `docs/features/01_model_management/`, `docs/integrations/02_vllm.md`, `docs/integrations/03_ollama.md`, `docs/integrations/04_llamacpp.md`. If any hard dependency is unavailable, Gateway Overview fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
On failure, no partial state from Gateway Overview is left visible. If Gateway Overview is a step inside an agent plan, the agent kernel's replanning logic (`docs/features/04_agent_kernel/10_replanning.md`) decides whether to retry, substitute a different approach, or surface the failure to the user, per `docs/runtime/14_resume_and_recovery.md`.

## 21. Observability requirements

Emits one structured log line per invocation (`level`, `event=inference_gateway.gateway_overview`, `actor_id`, `duration_ms`, `outcome`) and one metric `inference_gateway_gateway_overview_duration_seconds` (histogram) plus `inference_gateway_gateway_overview_total{outcome}` (counter), per `docs/features/25_observability/03_metrics.md`.

## 22. Audit requirements

Every invocation, successful or not, produces exactly one `AuditEvent` of type `inference_gateway.gateway_overview` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `Model`; `action` is `read`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).
...
Gateway Overview is done when all Acceptance criteria above pass in CI, the corresponding `docs/schemas/` and `docs/api/` entries match the implementation exactly, and the checklist in `docs/11_DEFINITION_OF_DONE.md` is satisfied.

## 27. Implementation notes

Implement `inference_gateway.gateway_overview` as a single service function called by both the HTTP route and any internal caller (e.g. the agent kernel) — do not duplicate the logic. Reuse the shared RBAC/policy check helper rather than writing a new role comparison; reuse the shared audit-emit helper rather than writing to the audit table directly.
...
**Success:** `admin` calls `POST /api/v1/inference-gateway/gateway-overview` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `inference_gateway.gateway_overview` audit event recorded.

**Denied:** `restricted` calls the same endpoint → `403`, `IG-4030`, audit event recorded with `outcome=denied`.

## 30. Edge cases

Empty/missing `payload` → `400` before any state is touched. Duplicate call with the same `idempotency_key` within the retry window → returns the original result, does not re-execute. Concurrent calls targeting the same `resource_id` → the later call observes the row-level lock and either waits or receives `409`, never a silent lost update. Dependency listed above is degraded but not fully down → Gateway Overview fails closed rather than guessing.
