...
Hard dependencies: `docs/integrations/13_opentelemetry.md`, `docs/integrations/14_prometheus.md`. If any hard dependency is unavailable, Observability Failures fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
**Success:** `admin` calls `POST /api/v1/observability/observability-failures` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `observability.observability_failures` audit event recorded.
