...
Hard dependencies: `docs/features/02_model_router/`, `docs/features/05_tool_gateway/`, `docs/features/16_human_approval/`, `docs/features/22_agent_memory/`. If any hard dependency is unavailable, Intent Interpretation fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
**Success:** `admin` calls `POST /api/v1/agent-kernel/intent-interpretation` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `agent_kernel.intent_interpretation` audit event recorded.
