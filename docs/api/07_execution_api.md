...
|---|---|---|---|---|
| `GET` | `/api/v1/tasks/{id}/plan` | (none) | Plan (`schemas/04_execution_schema.md`) | Returns the current AgentRun's plan. |
| `GET` | `/api/v1/tasks/{id}/steps/stream` | (none) | Server-Sent Events stream of PlanStep status updates | Streaming per `features/03_inference_gateway/07_streaming.md`. |

## Example — GET /api/v1/tasks/{id}/plan
