...
**Success:** `admin` calls `POST /api/v1/multimodal/vision-model-selection` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `multimodal.vision_model_selection` audit event recorded.
