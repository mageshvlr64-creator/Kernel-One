...
Hard dependencies: `docs/features/01_model_management/`, `docs/features/03_inference_gateway/`, `docs/features/21_policy_engine/`. If any hard dependency is unavailable, Capability Matching fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
Every invocation, successful or not, produces exactly one `AuditEvent` of type `model_router.capability_matching` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `Model`; `action` is `execute`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).
...
**Success:** `admin` calls `POST /api/v1/model-router/capability-matching` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `model_router.capability_matching` audit event recorded.
