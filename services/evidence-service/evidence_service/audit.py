...
Same contract knowledge-fabric implements: every invocation of an evidence op —
success, error, or denial — produces exactly one AuditEvent conforming to the
canonical schema. The tamper-evident chain (prev_event_hash) and INSERT-only
storage are audit-service's (Character 5, feature group 17); this module emits
schema-valid events into the sink it is given and never writes to a database.

``additionalProperties: false`` in the canonical schema — only the exact fields
below are ever emitted. ``actor_id='system'`` is allowed for internal jobs;
``session_id``/``correlation_id`` may be null.
"""
...
class InMemoryAuditSink:
    ...
    Preserves insert order and chains prev_event_hash like the canonical storage
    rules require, so the chain property is testable before the real service exists.
    """
    ...
    def emit(self, event: Dict[str, Any]) -> None:
        ...
        # Canonical hash input: the event sans prev_event_hash (documented here,
        # verified by audit-service's integrity job when it lands).
