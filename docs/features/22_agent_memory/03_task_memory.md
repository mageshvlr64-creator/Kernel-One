...
**Success:** `admin` calls `POST /api/v1/agent-memory/task-memory` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `agent_memory.task_memory` audit event recorded.
