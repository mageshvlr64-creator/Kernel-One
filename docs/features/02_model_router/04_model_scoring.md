...
Hard dependencies: `docs/features/01_model_management/`, `docs/features/03_inference_gateway/`, `docs/features/21_policy_engine/`. If any hard dependency is unavailable, Model Scoring fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
**Success:** `admin` calls `POST /api/v1/model-router/model-scoring` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `model_router.model_scoring` audit event recorded.
