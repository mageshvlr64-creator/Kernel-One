...
Hard dependencies: `docs/integrations/13_opentelemetry.md`, `docs/integrations/14_prometheus.md`. If any hard dependency is unavailable, Observability Overview fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
Resource: `SystemConfiguration` · Action: `read` · Risk level: **low**.
...
Every invocation, successful or not, produces exactly one `AuditEvent` of type `observability.observability_overview` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `SystemConfiguration`; `action` is `read`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).
...
**Success:** `admin` calls `POST /api/v1/observability/observability-overview` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `observability.observability_overview` audit event recorded.
