...
Resource: `Task` · Action: `read` · Risk level: **low**.
...
**Success:** `admin` calls `POST /api/v1/agent-memory/memory-retrieval` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `agent_memory.memory_retrieval` audit event recorded.
