...
**Success:** `admin` calls `POST /api/v1/spreadsheet-intelligence/table-analysis` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `spreadsheet_intelligence.table_analysis` audit event recorded.
