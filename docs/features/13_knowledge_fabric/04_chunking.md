...
Hard dependencies: `docs/features/10_document_ingestion/`, `docs/integrations/06_pgvector.md`. If any hard dependency is unavailable, Chunking fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
Every invocation, successful or not, produces exactly one `AuditEvent` of type `knowledge_fabric.chunking` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `Document`; `action` is `execute`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).
...
**Success:** `admin` calls `POST /api/v1/knowledge-fabric/chunking` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `knowledge_fabric.chunking` audit event recorded.
