...
| `inference-gateway/` | 1 — Foundation & Inference | 03 inference gateway (one provider path) | #7 | 36 | provider adapters (vLLM/Ollama/llama.cpp shapes), retry + fallback walk |
...
runs each service's suite in its own matrix job. Current repo total: **612
passing tests** (123 + 94 + 57 + 183 + 119 + 36), ruff clean.
