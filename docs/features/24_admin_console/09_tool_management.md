...
**Success:** `admin` calls `POST /api/v1/admin-console/tool-management` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `admin_console.tool_management` audit event recorded.
