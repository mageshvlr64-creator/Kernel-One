...
**Success:** `admin` calls `POST /api/v1/network-sovereignty/network-failures` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `network_sovereignty.network_failures` audit event recorded.
