...
> Root specification document · `docs/11_DEFINITION_OF_DONE.md`
> Previous: `10_DEPENDENCY_GRAPH.md` · Next: `12_GLOBAL_ACCEPTANCE_CRITERIA.md`
...
1. **Statement.** Definition of Done is authoritative for its topic across the entire Sovereign AI Workbench
...
2. **Cross-cutting application.** Every feature group under `docs/features/` is expected to be
   consistent with Definition of Done — if a reviewer finds a feature file that conflicts with this
...
3. **Enforcement.** Adherence to Definition of Done is part of `11_DEFINITION_OF_DONE.md` and
...
Changes to Definition of Done must be reflected in `docs/20_DECISION_LOG.md` with the date, the reason
