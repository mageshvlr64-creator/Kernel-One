...
Hard dependencies: `docs/features/10_document_ingestion/`, `docs/integrations/06_pgvector.md`. If any hard dependency is unavailable, Document Normalization fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
**Success:** `admin` calls `POST /api/v1/knowledge-fabric/document-normalization` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `knowledge_fabric.document_normalization` audit event recorded.
