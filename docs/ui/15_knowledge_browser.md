...
All roles with Document:read

## Key elements

- Document list with classification badges
- Upload button
- State indicator (UPLOADED..READY)
...
Empty: 'No documents in this workspace yet.' Loading: upload progress bar during ingestion. Error: 'Ingestion failed' with error code + retry. Permission-denied: documents above caller's clearance simply do not appear in the list (not shown-then-blocked).
