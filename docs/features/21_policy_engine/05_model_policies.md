...
Resource: `Policy` · Action: `execute` · Risk level: **high**.
...
**Success:** `admin` calls `POST /api/v1/policy-engine/model-policies` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `policy_engine.model_policies` audit event recorded.
