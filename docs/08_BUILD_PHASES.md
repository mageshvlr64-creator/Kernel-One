...
> Root specification document · `docs/08_BUILD_PHASES.md`
> Previous: `07_HARDWARE_AND_DEPLOYMENT_CONSTRAINTS.md` · Next: `09_BUILD_ORDER.md`

## Purpose

This document defines the coarse-grained phases the build is organized into, from skeleton to hardened release for the Sovereign AI Workbench (SIH26176).
...
1. **Statement.** Build Phases is authoritative for its topic across the entire Sovereign AI Workbench
...
2. **Cross-cutting application.** Every feature group under `docs/features/` is expected to be
   consistent with Build Phases — if a reviewer finds a feature file that conflicts with this
...
3. **Enforcement.** Adherence to Build Phases is part of `11_DEFINITION_OF_DONE.md` and
...
Changes to Build Phases must be reflected in `docs/20_DECISION_LOG.md` with the date, the reason
