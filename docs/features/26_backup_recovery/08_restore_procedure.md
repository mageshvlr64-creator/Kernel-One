...
Hard dependencies: `docs/integrations/05_postgresql.md`, `docs/integrations/07_minio.md`. If any hard dependency is unavailable, Restore Procedure fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
**Success:** `admin` calls `POST /api/v1/backup-recovery/restore-procedure` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `backup_recovery.restore_procedure` audit event recorded.
