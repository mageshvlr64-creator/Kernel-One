...
> Root specification document · `docs/09_BUILD_ORDER.md`
> Previous: `08_BUILD_PHASES.md` · Next: `10_DEPENDENCY_GRAPH.md`

## Purpose

This document defines the fine-grained order features must be implemented in, given their dependencies for the Sovereign AI Workbench (SIH26176).
...
1. **Statement.** Build Order is authoritative for its topic across the entire Sovereign AI Workbench
...
2. **Cross-cutting application.** Every feature group under `docs/features/` is expected to be
   consistent with Build Order — if a reviewer finds a feature file that conflicts with this
...
3. **Enforcement.** Adherence to Build Order is part of `11_DEFINITION_OF_DONE.md` and
...
Changes to Build Order must be reflected in `docs/20_DECISION_LOG.md` with the date, the reason
