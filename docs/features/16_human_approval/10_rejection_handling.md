...
**Success:** `admin` calls `POST /api/v1/human-approval/rejection-handling` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `human_approval.rejection_handling` audit event recorded.
