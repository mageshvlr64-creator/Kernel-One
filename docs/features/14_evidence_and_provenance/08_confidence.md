...
Each row above maps to a named entry in `docs/failures/` for this subsystem; no failure produced by Confidence is allowed to exist without a corresponding documented failure mode.

## 18. Retry behavior

Confidence is treated as retryable: up to 3 attempt(s) with exponential backoff starting at 284ms, per `docs/runtime/11_retry_policy.md`. Retries are only issued for `503`/`504`-class failures, never for `400`/`403`/`409`.

## 19. Timeout behavior

Maximum execution time: **13s**, enforced per `docs/runtime/12_timeout_policy.md`. On timeout, any in-flight subprocess, container, or DB transaction opened by Confidence is terminated/rolled back and the caller receives the `504` error from the table above.
...
Every invocation, successful or not, produces one audit event of type `evidence_and_provenance.confidence` containing `actor_id`, `workspace_id`, `resource_id`, a hash of the payload (never the raw payload if it may contain sensitive content), and `outcome`, per `docs/features/17_audit/03_event_schema.md`.
...
**Success:** `admin` calls `POST /api/v1/evidence-and-provenance/confidence` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "<next-state>"}`, one `evidence_and_provenance.confidence` audit event recorded.
