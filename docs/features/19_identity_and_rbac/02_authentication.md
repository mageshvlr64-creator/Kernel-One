...
Every invocation, successful or not, produces exactly one `AuditEvent` of type `identity_and_rbac.authentication` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `User`; `action` is `execute`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).
...
**Success:** `admin` calls `POST /api/v1/identity-and-rbac/authentication` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `identity_and_rbac.authentication` audit event recorded.
