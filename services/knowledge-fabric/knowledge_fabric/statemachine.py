...
this module is that guard for the Document machine as triggerable from the Knowledge Fabric
service. Only its own transitions are offered here: INDEXING -> READY (event document.indexed)
and INDEXING -> FAILED (content-level failure). Every other transition belongs to another
component (document-pipeline, ocr) and raises RegistryError(RESOURCE_CONFLICT) — never a
silent allowance.
"""
...
OWNER_ALIASES = {"knowledge_fabric"}  # owners triggerable from THIS service
