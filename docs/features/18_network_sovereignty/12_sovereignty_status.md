...
Sovereignty Status does not perform its own permission check logic — it calls the shared RBAC layer (`docs/features/19_identity_and_rbac/05_permissions.md`). It does not decide routing or model selection itself. It does not write directly to the audit table — it emits an event that `docs/features/17_audit/` consumes. It makes zero outbound network calls outside the active network mode.

## 4. User-facing behavior

From the workbench UI, sovereignty status is triggered by the corresponding action in the relevant panel (see `docs/ui/`) or issued programmatically via `POST /api/v1/network-sovereignty/sovereignty-status`. On success the UI reflects the new state within one refresh cycle (≤ 1s). On failure the UI shows a specific, human-readable message mapped from the error code table below — never a raw stack trace or a silent no-op.

## 5. System behavior

1) Request arrives at `/api/v1/network-sovereignty/sovereignty-status` and passes through auth + RBAC + policy checks. 2) The `Network Sovereignty` service validates the payload against its schema. 3) The operation is applied to the `network_events` store inside a single transaction. 4) An audit event `network_sovereignty.sovereignty_status` is emitted. 5) The response is returned to the caller. Steps 3-4 are atomic: if step 4 cannot be guaranteed, step 3 is rolled back rather than leaving an unaudited state change.
...
On success: the `network_events` row reflects the new state; exactly one audit event `network_sovereignty.sovereignty_status` exists for this invocation; any cache or index derived from `network_events` is invalidated or updated in the same logical operation. On failure: no partial write is visible to any other caller.

## 10. Data structures

Canonical shape lives in `docs/schemas/` (see the schema file matching `network_events`) and `docs/domain/` for the entity-level definition. This file does not redefine those fields — it only specifies which of them `Sovereignty Status` reads or writes.

## 11. API contracts

`POST /api/v1/network-sovereignty/sovereignty-status` — create/invoke. `GET /api/v1/network-sovereignty/sovereignty-status/{id}` — read current state. Full request/response schema, headers, and pagination (where applicable) are defined in `docs/api/` under the entry for `Network Sovereignty`. Auth: Bearer session token, required on every call, no anonymous access.

## 12. Internal interfaces

Backend module: `network_sovereignty_sovereignty_status_service`, exposing a single entry point `handle_sovereignty_status(context, payload) -> Result`. Callers (e.g. the agent kernel's tool selection step, or the admin console) invoke this function directly for in-process calls; the HTTP route above is a thin wrapper around the same function — there is exactly one implementation, not two.

## 13. State transitions

Sovereignty Status moves its resource through the states defined in the relevant `docs/runtime/` state machine file for `Network Sovereignty`. It is only permitted to trigger the specific transition(s) relevant to "sovereignty status" — any other transition is a defect, and the state machine layer must reject it rather than silently allow it.

## 14. Dependencies

Hard dependencies: `docs/features/17_audit/`. If any hard dependency is unavailable, Sovereignty Status fails closed per Failure modes — it does not silently degrade to an unsafe default.

## 15. Security requirements

All input is treated as untrusted and validated against its schema before use. No input is interpolated into a shell command, SQL string, or filesystem path without going through the sanitization described in `docs/security/09_path_traversal.md` and `docs/security/17_malicious_documents.md` where relevant. If sovereignty status processes agent- or model-generated content, it is treated as untrusted per `docs/security/05_prompt_injection.md`.
...
Each row above maps to a named entry in `docs/failures/` for this subsystem; no failure produced by Sovereignty Status is allowed to exist without a corresponding documented failure mode.

## 18. Retry behavior

Sovereignty Status is classified high-risk and is **not** automatically retried — a failed attempt surfaces to the caller (or the agent kernel's replanning step, `docs/features/04_agent_kernel/10_replanning.md`) for an explicit new decision, per `docs/runtime/11_retry_policy.md`.

## 19. Timeout behavior

Maximum execution time: **37s**, enforced per `docs/runtime/12_timeout_policy.md`. On timeout, any in-flight subprocess, container, or DB transaction opened by Sovereignty Status is terminated/rolled back and the caller receives the `504` error from the table above.

## 20. Recovery behavior

On failure, no partial state from Sovereignty Status is left visible. If Sovereignty Status is a step inside an agent plan, the agent kernel's replanning logic (`docs/features/04_agent_kernel/10_replanning.md`) decides whether to retry, substitute a different approach, or surface the failure to the user, per `docs/runtime/14_resume_and_recovery.md`.

## 21. Observability requirements

Emits one structured log line per invocation (`level`, `event=network_sovereignty.sovereignty_status`, `actor_id`, `duration_ms`, `outcome`) and one metric `network_sovereignty_sovereignty_status_duration_seconds` (histogram) plus `network_sovereignty_sovereignty_status_total{outcome}` (counter), per `docs/features/25_observability/03_metrics.md`.

## 22. Audit requirements

Every invocation, successful or not, produces one audit event of type `network_sovereignty.sovereignty_status` containing `actor_id`, `workspace_id`, `resource_id`, a hash of the payload (never the raw payload if it may contain sensitive content), and `outcome`, per `docs/features/17_audit/03_event_schema.md`.
...
Minimum coverage: one unit test per row in the Failure modes table above, one integration test for the full success path through `/api/v1/network-sovereignty/sovereignty-status`, and one permission test per role in the table above (three roles × allow/deny = at least 3 assertions), per `docs/testing/01_testing_strategy.md`.

## 25. Acceptance criteria

- [ ] `POST /api/v1/network-sovereignty/sovereignty-status` succeeds for an `admin` caller with a valid payload and returns the shape in Outputs.
- [ ] The same call is denied per the Permission requirements table for a disallowed role, with the correct error code.
- [ ] Exactly one `network_sovereignty.sovereignty_status` audit event is produced per invocation, success or failure.
...
**Success:** `admin` calls `POST /api/v1/network-sovereignty/sovereignty-status` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "<next-state>"}`, one `network_sovereignty.sovereignty_status` audit event recorded.

**Denied:** `restricted` calls the same endpoint → `403`, `NS-4030`, audit event recorded with `outcome=denied`.

## 30. Edge cases

Empty/missing `payload` → `400` before any state is touched. Duplicate call with the same `idempotency_key` within the retry window → returns the original result, does not re-execute. Concurrent calls targeting the same `resource_id` → the later call observes the row-level lock and either waits or receives `409`, never a silent lost update. Dependency listed above is degraded but not fully down → Sovereignty Status fails closed rather than guessing.
