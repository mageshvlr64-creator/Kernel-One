...
Hard dependencies: `docs/integrations/13_opentelemetry.md`, `docs/integrations/14_prometheus.md`. If any hard dependency is unavailable, Structured Logging fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
Every invocation, successful or not, produces exactly one `AuditEvent` of type `observability.structured_logging` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `SystemConfiguration`; `action` is `execute`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).
...
**Success:** `admin` calls `POST /api/v1/observability/structured-logging` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `observability.structured_logging` audit event recorded.
