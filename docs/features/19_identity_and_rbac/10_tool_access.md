...
Each row above maps to a named entry in `docs/failures/` for this subsystem; no failure produced by Tool Access is allowed to exist without a corresponding documented failure mode.

## 18. Retry behavior

Tool Access is classified high-risk and is **not** automatically retried — a failed attempt surfaces to the caller (or the agent kernel's replanning step, `docs/features/04_agent_kernel/10_replanning.md`) for an explicit new decision, per `docs/runtime/11_retry_policy.md`.

## 19. Timeout behavior

Maximum execution time: **13s**, enforced per `docs/runtime/12_timeout_policy.md`. On timeout, any in-flight subprocess, container, or DB transaction opened by Tool Access is terminated/rolled back and the caller receives the `504` error from the table above.
...
Every invocation, successful or not, produces one audit event of type `identity_and_rbac.tool_access` containing `actor_id`, `workspace_id`, `resource_id`, a hash of the payload (never the raw payload if it may contain sensitive content), and `outcome`, per `docs/features/17_audit/03_event_schema.md`.
...
**Success:** `admin` calls `POST /api/v1/identity-and-rbac/tool-access` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "<next-state>"}`, one `identity_and_rbac.tool_access` audit event recorded.
