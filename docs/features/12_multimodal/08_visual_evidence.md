...
**Success:** `admin` calls `POST /api/v1/multimodal/visual-evidence` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `multimodal.visual_evidence` audit event recorded.
