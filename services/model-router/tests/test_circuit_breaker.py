...
class FakeClock:
    """Controllable monotonic clock: tests advance it instead of sleeping."""
...
class TestCircuitBreakerTrips:
    ...
    def test_success_during_half_open_resets(self):
        clock = FakeClock()
        cb = _make_breaker(
            failure_threshold=2, half_open_probe_seconds=15.0, clock=clock.now
        )
        cb.record_failure("model-a")
        cb.record_failure("model-a")
        assert cb.get_state("model-a") == CircuitState.OPEN

        clock.advance(16.0)  # past the probe window — no real sleep
        assert cb.allow_request("model-a") is True
