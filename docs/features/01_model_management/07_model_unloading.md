...
Hard dependencies: `docs/features/03_inference_gateway/`, `docs/features/21_policy_engine/`. If any hard dependency is unavailable, Model Unloading fails closed per Failure modes — it does not silently degrade to an unsafe default.

**Traceability:** this feature implements `REQ-AI-003` (see `docs/03_REQUIREMENTS.md`).
...
Resource: `Model` · Action: `delete` · Risk level: **low**.
...
Every invocation, successful or not, produces exactly one `AuditEvent` of type `model_management.model_unloading` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `Model`; `action` is `delete`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).

## 23. Performance requirements

p95 < 200ms for registry reads (in-memory/SQLite cache), < 5s for a model load/unload operation. Measured and enforced per `docs/performance/01_performance_requirements.md` on the reference hardware profile in `docs/07_HARDWARE_AND_DEPLOYMENT_CONSTRAINTS.md`. **Status: DESIGN LIMIT** — this budget is an engineering target chosen for demo usability on the reference hardware profile, not yet a benchmark-verified measurement (see `docs/20_DECISION_LOG.md` DEC-014 for the pending validation step). Treat it as a target to test against, not a guaranteed SLA, until DEC-014 is resolved.
...
**Success:** `admin` calls `POST /api/v1/model-management/model-unloading` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `model_management.model_unloading` audit event recorded.
