...
Hard dependencies: `docs/features/03_inference_gateway/`, `docs/features/21_policy_engine/`. If any hard dependency is unavailable, Model Approval fails closed per Failure modes — it does not silently degrade to an unsafe default.
...
Resource: `Model` · Action: `approve` · Risk level: **low**.
...
Every invocation, successful or not, produces exactly one `AuditEvent` of type `model_management.model_approval` conforming to the canonical schema in `docs/schemas/15_audit_event_schema.md` — this file does not redefine the `AuditEvent` field set. `resource_type` is `Model`; `action` is `approve`; `classification` is populated when the resource carries one (`docs/features/20_data_classification/`).
...
**Success:** `admin` calls `POST /api/v1/model-management/model-approval` with a valid payload → `200 OK`, `{"status": "ok", "resource_id": "…", "state": "the resulting state (see `docs/runtime/_state_machines_canonical.md`)"}`, one `model_management.model_approval` audit event recorded.
