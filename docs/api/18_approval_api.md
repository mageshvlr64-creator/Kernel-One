...
|---|---|---|---|---|
| `GET` | `/api/v1/approvals` | query: ?state&cursor&limit | paginated Approval[] | Administrator/SecurityOfficer see all; others see only their own requests. |
| `POST` | `/api/v1/approvals/{id}/decide` | ApprovalDecision (`schemas/12_approval_schema.md`) | Approval | Requires Administrator or SecurityOfficer. `RESOURCE_CONFLICT` if already decided/expired. |

## Example — GET /api/v1/approvals
